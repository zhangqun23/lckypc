...
public  class BusNeedDaoImpl implements BusNeedDao {
	...
	// 判断查找关键字是否为空
		if (null != searchKey) {
			if(null != butrState){
			selectSql += " and butr_state = " + butrState + " and ( bune_tel like '%" + searchKey + "%' or bune_goal_pla like '%" + searchKey + "%')";
			}
			else selectSql +=  " and ( bune_tel like '%" + searchKey + "%' or bune_goal_pla like '%" + searchKey + "%')";
			}
		else {
			if(null != butrState){
				selectSql += " and butr_state = " + butrState;
			}
		}
	...
	public Integer countBuneTotal(String butrState,String searchKey) {
		...
		if (null != searchKey) {
			if(null != butrState){
			countSql += " and butr_state = " + butrState + " and (bune_tel like '%" + searchKey + "%' or bune_goal_pla like '%" + searchKey + "%')";
			}
			else countSql += " and (bune_tel like '%" + searchKey + "%' or bune_goal_pla like '%" + searchKey + "%')";
			}
		else {
			if(null != butrState){
				countSql += " and butr_state = " + butrState;
			}
		}
